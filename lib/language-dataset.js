import { Constants, Feature, LanguageModelFactory as LMF } from 'alpheios-data-models'
...
export default class LanguageDataset {
  ...
  setInflectionData (inflection, lemma) {
    ...
    if (this.languageID === Constants.LANG_GREEK) {
      GreekParadigmDataset.setParadigmInflectionData(inflection)
    }
  ...
  createInflectionSet (pofsValue, inflections, options) {
    ...
    if (inflectionSet.isImplemented) {
      ...
      GreekParadigmDataset.createParadigmInflectionSet(pofsValue, inflections, inflectionSet)

      const sourceSet = this.pos.get(pofsValue)
      if (!sourceSet && !GreekParadigmDataset.sourceSet(pofsValue)) {
        // There is no source data for this part of speech
      ...
      */
      ...
      if (inflectionSet.hasTypes) {
        const finalSourceSet = sourceSet || GreekParadigmDataset.sourceSet(pofsValue)
