...
export default class LatinLanguageDataset extends LanguageDataset {
  ...
  isIrregular (inflection) {
    ...
    if (this.irregularLemmas.has(pofs)) {
      if (inflection[Feature.types.conjugation] && inflection[Feature.types.conjugation].value === Constants.TYPE_IRREGULAR) {
        ...
        return this.irregularLemmas.get(pofs).some(item => inflection.modelCompareWords(item.word, inflection[Feature.types.word].value))
      }
    }
    return false
  }
  ...
  isImplemented (inflection) {
    ...
    return Boolean(
      ...
      this.irregularLemmas.get(pofs).some(item => inflection.modelCompareWords(item.word, inflection[Feature.types.word].value))
    )
  }
  ...
  getMatchingIrregularLemmas (inflections) {
    ...
    for (const inflection of inflections) {
      const pofs = inflection[Feature.types.part].value
      if (this.irregularLemmas.has(pofs)) {
        const lemma = this.irregularLemmas.get(pofs).find(item => inflection.modelCompareWords(item.word, inflection[Feature.types.word].value))
        if (lemma) {
          lemmas.push(lemma)
        }
      }
