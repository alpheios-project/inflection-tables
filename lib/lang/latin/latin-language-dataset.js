...
export default class LatinLanguageDataset extends LanguageDataset {
  constructor () {
    ...
    this.features.get(Feature.types.conjugation).getImporter()
      .map(this.constructor.constants.CONJ_ANY, [Constants.ORD_1ST, Constants.ORD_2ND, Constants.ORD_3RD, Constants.ORD_4TH, Constants.TYPE_IRREGULAR])
  ...
  checkIrregularVerb (inflection) {
    if ([Constants.POFS_VERB, Constants.POFS_VERB_PARTICIPLE].includes(
      inflection[Feature.types.part].value) && inflection[Feature.types.word]
    ) {
  ...
  static getObligatoryMatchList (inflection) {
    if (inflection.constraints.irregularVerb || Constants.POFS_SUPINE || Constants.POFS_GERUNDIVE) {
