...
export default class LatinLanguageDataset extends LanguageDataset {
  ...
  addVerbForms (partOfSpeech, data) {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (principalParts) {
        features.push(this.features.get(Feature.types.hdwd).createFromImporter(principalParts))
  ...
  getVerbsIrregularLemma (verbForm) {
    let formatedFormsToObjects = this.pos.get(Constants.POFS_VERB).types.get(Form).items.map(item => { return { val: item.value, lemma: item.features[Feature.types.hdwd].value, fullForm: item.features[Feature.types.fullForm].value } })
  ...
  checkIrregularVerb (inflection) {
    return (inflection.hasFeatureValue(Feature.types.part, Constants.POFS_VERB) && this.getVerbsIrregularLemma(inflection[Feature.types.fullForm]))
  }
