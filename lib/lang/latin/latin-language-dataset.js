...
export default class LatinLanguageDataset extends LanguageDataset {
  ...
  getVerbsIrregularLemma (verbForm) {
    let res = this.pos.get(Constants.POFS_VERB).types.get(Form).items.map(item => { return { val: item.value, lemma: item.features[Feature.types.hdwd].value, fullForm: item.features[Feature.types.fullForm].value } })
    let res1 = {}
  ...
  checkIrregularVerb (inflection) {
    if (inflection.hasFeatureValue(Feature.types.part, Constants.POFS_VERB) && this.getVerbsIrregularLemma(inflection[Feature.types.fullForm])) {
      return true
    }
