...
export default class LatinLanguageDataset extends LanguageDataset {
  ...
  addVerbForms (partOfSpeech, data) {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (lemma) {
        features.push(this.features.get(Feature.types.hdwd).createFromImporter(lemma))
  ...
  checkIrregularVerb (inflection) {
    if (inflection.hasFeatureValue(Feature.types.part, Constants.POFS_VERB) && inflection[Feature.types.word]) {
      return this.verbsIrregularLemmas.indexOf(inflection[Feature.types.word].value) > -1
    }
    return false
  }
