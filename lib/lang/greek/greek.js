...
const parts = dataSet.defineFeatureType(Feature.types.part, ['noun', 'adjective', 'verb']);
const numbers = dataSet.defineFeatureType(Feature.types.number, ['singular', 'dual', 'plural']);
numbers.addImporter(importerName)
    .map('singular', numbers.singular)
    .map('dual', numbers.dual)
    .map('plural', numbers.plural);
const cases = dataSet.defineFeatureType(Feature.types.grmCase, ['nominative', 'genitive', 'dative', 'accusative', 'vocative']);
...
const declensions = dataSet.defineFeatureType(Feature.types.declension, ['first', 'second', 'third']);
declensions.addImporter(importerName)
    .map('1st', declensions.first)
    .map('2nd', declensions.second)
    .map('3rd', declensions.third);
const genders = dataSet.defineFeatureType(Feature.types.gender, ['masculine', 'feminine', 'neuter']);
...
const types = dataSet.defineFeatureType(Feature.types.type, ['regular', 'irregular']);
...
const footnotes = dataSet.defineFeatureType(Feature.types.footnote, []);
