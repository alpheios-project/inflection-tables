...
import articleSuffixesCSV from '@lib/lang/greek/data/article/suffixes.csv'
...
export default class GreekLanguageDataset extends LanguageDataset {
  ...
  loadData () {
    ...
    suffixes = papaparse.parse(articleSuffixesCSV, {})
  ...
  static getObligatoryMatchList (inflection) {
    ...
    } else if (inflection.hasFeatureValue(Feature.types.part, Constants.POFS_NUMERAL) || inflection.hasFeatureValue(Feature.types.part, Constants.POFS_ARTICLE)) {
  ...
  static getOptionalMatchList (inflection) {
    ...
    if (inflection.hasFeatureValue(Feature.types.part, Constants.POFS_NUMERAL)) {
      ...
      featureOptions = [
        ...
        new GroupFeatureType(genders, 'Gender'),
